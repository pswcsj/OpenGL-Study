...
int main()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);//버퍼(데이터)들을 VBO에저장 (버퍼 종류, 버퍼 사이즈, 버퍼 시작 주소, 버퍼 사용 방법)
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);//EMO를 바인딩 GL_ELEMENT_ARRAY_BUFFER는  GL_ARRAY_BUFFER의 indices를 나타내는 데이터에 쓰임
